...
// 信道状态枚举
const STATUS_CLOSED = Tunnel.STATUS_CLOSED = 'CLOSED';
const STATUS_CONNECTING = Tunnel.STATUS_CONNECTING = 'CONNECTING';
const STATUS_ACTIVE = Tunnel.STATUS_ACTIVE = 'ACTIVE';
const STATUS_RECONNECTING = Tunnel.STATUS_RECONNECTING = 'RECONNECTING';

// 错误类型枚举
const ERR_CONNECT_SERVICE = Tunnel.ERR_CONNECT_SERVICE = 1001;
const ERR_CONNECT_SOCKET = Tunnel.ERR_CONNECT_SOCKET = 1002;
const ERR_RECONNECT = Tunnel.ERR_RECONNECT = 2001;
const ERR_SOCKET_ERROR = Tunnel.ERR_SOCKET_ERROR = 3001;
...
// 断线重连最多尝试 5 次
const DEFAULT_MAX_RECONNECT_TRY_TIMES = 5;

// 每次重连前，等待时间的增量值
const DEFAULT_RECONNECT_TIME_INCREASE = 1000;
...
function Tunnel(serviceUrl) {
  ...
  //=========================================================================
  const preservedEventTypes = 'connect,close,reconnecting,reconnect,error'.split(',');
  ...
  function dispatchEvent(eventType, eventPayload?) {
    eventHandlers.forEach(handler => {
      const handleType = handler[0];
      const handleFn = handler[1];
  ...
  function dispatchEscapedEvent(eventType, eventPayload) {
    if (preservedEventTypes.includes(eventType)) {
      eventType = `@${eventType}`;
    }
   ...
   * 数据包推送到信道
   */
  function sendPacket({type, content}) {
    const encodedPacket = [type];

    if (content) {
      encodedPacket.push(JSON.stringify(content));
    }
  ...
  function resolvePacket(raw) {
    const packetParts = raw.split(':');
    const packetType = packetParts.shift();
    const packetContent = packetParts.join(':') || null;
    const packet: any = { type: packetType };
   ...
   */
  function handleMessagePacket({content}) {
   ...
   */
  function handleTimeoutPacket({content}) {
    const timeout = content * 1000;
    /* istanbul ignore else */
    if (!isNaN(timeout)) {
  ...
  // 已经重连失败的次数
  let reconnectTryTimes = 0;

  // 最多允许失败次数
  const maxReconnectTryTimes = Tunnel.MAX_RECONNECT_TRY_TIMES || DEFAULT_MAX_RECONNECT_TRY_TIMES;

  // 重连前等待的时间
  let waitBeforeReconnect = 0;

  // 重连前等待时间增量
  const reconnectTimeIncrease = Tunnel.RECONNECT_TIME_INCREASE || DEFAULT_RECONNECT_TIME_INCREASE;
  ...
  function closeSocket(emitClose = false) {
    if (isActive() && emitClose) {
      emitClosePacket();
    }
